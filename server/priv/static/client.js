...
class WebSocket extends CustomType {
}
class ServerSentEvents extends CustomType {
}
...
function main() {
  let app = simple((_) => {
    ...
    return div(toList([styles(toList([["height", "100vh"]]))]), toList([
      element4(toList([
        route("/ws"),
  ...
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH, "client", 20, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 461, end: 510, pattern_start: 472, pattern_end: 477 });
  }
  return;
}
